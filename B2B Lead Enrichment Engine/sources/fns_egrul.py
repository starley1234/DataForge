...
class FNSEgrulClient:
    ...
    Поддерживает:
    - Поиск по ИНН юридических лиц (10 знаков) и ИП (12 знаков)
    - Поиск по ОГРН / ОГРНИП
    - Поиск по наименованию компании
    - Извлечение ФИО руководителя, должности, юр. адреса, выписки PDF, email и ОКВЭД.
    """
    ...
    def search_by_query(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Поиск в ЕГРЮЛ по запросу '{clean_query}': {e}")
        return []

    def fetch_company(self, query: str) -> Optional[Company]:
        """
        Полное извлечение карточки компании / ИП и первого лица из ЕГРЮЛ / ЕГРИП.
        """
