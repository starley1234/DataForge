...
class BatchProcessor:
    ...
    def parse_file_to_items(self, file_bytes: bytes, filename: str) -> List[str]:
        ...
        try:
            ...
            # Поиск колонки с ИНН или названиями
            target_col = None
            for col in df.columns:
                c_clean = str(col).lower().strip()
                if any(k in c_clean for k in ["инн", "inn", "огрн", "ogrn", "организация", "компания", "наименование", "домен", "domain"]):
                    target_col = col
                    break

            if target_col is not None:
                series = df[target_col].dropna().astype(str)
                items = [s.strip() for s in series if s.strip() and s.strip() != "nan"]
            else:
                # Если колонка не определена, берем первую колонку
    ...
    def start_batch_enrichment(
        ...
        clean_items = list(dict.fromkeys([it.strip() for it in items if it and it.strip()]))
    ...
    def _process_batch_job(
        ...
        for idx, item in enumerate(items, start=1):
            ...
            try:
                if task_type == "domain":
                    # Обогащение напрямую по сайту/домену
        ...
        try:
            ...
            if db_task:
                ...
                db_task.processed_items = len(items)
